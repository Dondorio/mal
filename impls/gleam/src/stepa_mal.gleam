...
import types.{
  type MalRet, type MalType, Bool, Func, HashMap, List, String, Symbol, Vector,
}
...
pub fn main() -> Nil {
  ...
  case argv.load().arguments {
    ...
    _ -> {
      let _ = rep("(println (str \"Mal [\" *host-language* \"]\"))", env)
      let _ = loop(env)
      Nil
    }
  }
...
fn eval_list(first, rest, env) -> Result(MalType, types.Error) {
  case first {
    ...
    Symbol("try*") -> try_special(rest, env)
...
fn try_special(rest, env) {
  case rest {
    [a] -> eval(a, env)
    [a, List([Symbol("catch*"), Symbol(sym), catch], _)] ->
      case eval(a, env) {
        Ok(res) -> Ok(res)
        Error(err) -> {
          let catch_env = env.into_outer(env)
          let v = case err {
            types.Throw(ast) -> ast
            _ -> String(printer.pr_err(err))
          }
          env.set(env, sym, v)

          eval(catch, catch_env)
        }
      }
    _ -> types.wrong_type_err("any, (catch* symbol, any)?", rest)
  }
}
